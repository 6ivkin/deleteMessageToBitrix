...
//    {
//        $query = "Select CONCAT(user.last_name, ' ', user.name) as name, massages.massage as count
//                  From b_im_massage as massages
//                  Join b_user as user on user.id = massages.author_id
//                  Group by author_id ";
//        $stmt = self::getDB()->prepare($query);
//        $stmt->execute();
//        try {
...
//    }
    ...
    {
        $itog = [];
        $query = "Select CONCAT(user.last_name, ' ', user.name) as name, user.id as id 
                From b_user as user";
        $stmt = self::getDB()->prepare($query);
        $stmt->execute();
        $ids = $stmt->fetchAll();
        ...
        foreach ($ids as $id) {
            $query = "Select massage from b_im_massage
